...
请确保分析基于最新的市场数据，用中文回答，保持专业、客观的分析风格。"""
    ...
    def _parse_smart_response(self, response: str) -> SmartAnalysisReport:
        ...
        if isinstance(sections["key_factors"], str):
            ...
            sections["key_factors"] = (
                factors if factors else ["市场供需变化", "宏观经济影响", "地缘政治因素"]
            )
        ...
        return SmartAnalysisReport(
            ...
            if isinstance(market_overview, str) and market_overview
            else "暂无数据",
            recent_trend=recent_trend
            if isinstance(recent_trend, str) and recent_trend
            else "暂无数据",
            key_factors=sections["key_factors"]
            if isinstance(sections["key_factors"], list)
            else ["暂无数据"],
            price_prediction=price_prediction
            if isinstance(price_prediction, str) and price_prediction
            else "暂无预测",
            buy_timing=buy_timing
            if isinstance(buy_timing, str) and buy_timing
            else "建议观望",
            recommendation=recommendation
            if isinstance(recommendation, str) and recommendation
            else "建议谨慎操作",
            risk_warning=risk_warning
            if isinstance(risk_warning, str) and risk_warning
            else "投资有风险，入市需谨慎",
    ...
    def _save_section(
        self, sections: dict[str, str | list[str]], section: str, content: list[str]
    ):
        """保存章节内容"""
        text = "\n".join(content).strip()
...
def create_llm_provider(provider: str | None = None) -> LLMProvider:
    ...
    if active_provider:
        ...
        if (
            "anthropic" in active_provider.base_url.lower()
            or "claude" in active_provider.name.lower()
        ):
            return AnthropicProvider(
                api_key=active_provider.api_key, model=config.active_model or None
            )
